...
function hiddenProductsFromStorefront(orders) {
	orders.items.forEach(function (entry){
		entry.items.forEach(function(entry){
			...
			$(element).children('.grid-product__wrap').children('.grid-product__wrap-inner').children('.grid-product__button').html(`<a href="${entry.files[0].customerUrl}"> <div class="download__wrapper"> <button class="download__btn" onclick="openProductDetail(${entry.productId})"> <span class="download__text">Скачать</span> </button> </div></a>`);
